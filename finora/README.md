...
Para deploy em produção, defina a variável `FINORA_FRONTEND_URL` com a URL pública do front-end — ela será adicionada automaticamente às origens permitidas.
...
3. Conecte o repositório e selecione **Docker** como runtime
4. Defina o **Dockerfile Path** como `./backend/Dockerfile`
5. Configure as variáveis de ambiente no painel do Render:

```
FINORA_DB_PASSWORD   → senha do banco PostgreSQL em produção
FINORA_JWT_SECRET    → chave Base64 com mínimo 32 bytes
FINORA_FRONTEND_URL  → URL pública do front-end (ex: https://finora.vercel.app)
```

> O arquivo `backend/render.yaml` na raiz do repositório pode ser usado para configurar o serviço automaticamente via Infrastructure as Code do Render.

### Front-end — Vercel

1. Acesse [vercel.com](https://vercel.com) e importe o repositório
2. Defina o **Root Directory** como `frontend/client`
3. O comando de build é `npm run build` e o diretório de saída é `dist`
4. Configure a variável de ambiente:

```
VITE_API_URL → URL pública do back-end (ex: https://finora-api.onrender.com)
```

### Ordem de configuração

1. Suba o banco PostgreSQL em produção (Render PostgreSQL, Supabase, Neon, etc.)
2. Faça deploy do back-end e obtenha a URL pública
3. Faça deploy do front-end informando a URL do back-end em `VITE_API_URL`
4. Volte ao back-end e configure `FINORA_FRONTEND_URL` com a URL do front-end
5. Teste o fluxo completo: cadastro → login → dashboard
